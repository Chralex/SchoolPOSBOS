...
public class DatabaseInitializer {
	...
	public static boolean IncompleteModel(DatabaseConnection db, DatabaseModel model) throws FieldNotInitializedException, SQLException {
		if (model.tables == null || model.tables.size() == 0)
			throw new FieldNotInitializedException(model);
		
		for (Table table : model.tables) {
			ResultSet result = db.connection.createStatement().executeQuery("SHOW TABLES LIKE '" + table.getClass().getSimpleName() + "'");
